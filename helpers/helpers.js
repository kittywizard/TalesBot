...
const func = require('./randomNum.js');
const changeTo = require('./titlecase.js');

module.exports = {
 displayData: (num = 1, flav = '') => {
    
    let flavorSelector;

    if(flav != ''){
      //take custom flavor and pull it instead of using the rando number. 
      data.forEach((flavor, index) => {
        if(flavor.flavor == changeTo.titlecaseWord(flav)) {
          flavorSelector = index;
        }
      });
      
      //error handling
      if(flavorSelector == undefined) {
        return [
          {
            name: "Error!",
            value: `That flavor (${flav}) doesn't match anything in our database. Please try again.`
          }
        ]
      }
      
    } else {
       flavorSelector = func.randomNum(0, data.length - 1);
    }
    ...
    for (let i = 0; i < num; i++) {  
       let promptSelector = func.randomNum(0, 29); //almost all of them are 30 flavors - with a few exceptions
        let messageObj = {
          flavor: data[flavorSelector].flavor,
          theme: data[flavorSelector].theme,
          number: promptSelector + 1,
          prompt: data[flavorSelector].prompts[promptSelector]
        };
  ...
  toppingGenerator: (toppings) => {
      const toppingNum = func.randomNum(0, toppings.length - 1); 
      return toppings[toppingNum];
  }
